...
def plot_dose_distribution(blood_dose_total, dose_contributions, mean_blood_dose=None):
    ...
    np.save(os.path.join(dir_path, 'DVH_doses.npy'), (bins1[:-1] + bins1[1:]) / 2)
    np.save(os.path.join(dir_path, 'DVH_volume_fractions.npy'),
        np.histogram(blood_dose_total.dose, bins=bins1, density=True)[0] * np.diff(bins1))
